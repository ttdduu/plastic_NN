...
class BlockLC(nn.Module):
    ...
    def forward(self, x):
       ...
       #  x = gamma * (x * Nx) + x
       #  #x = gamma * (x * Nx) + beta # and beta should be 0, being 0.9 of itself each epoch
       #  
...
class CustomConvNeXtAttoLC(BaseModel):
    def __init__(self, config, start_from_pretrained_non_lc_weights: bool = True):
        ...
        # Zero and freeze GRN beta parameters
        for name, param in self.named_parameters():
            if 'beta' in name:  # GRN beta parameters
                param.data.zero_()
                param.requires_grad = False
                print(f"Zeroed and frozen GRN beta: {name}")
    ...
    def _build_stages(self):
        ...
        if hasattr(self.config, 'logpolar_apply') and self.config.logpolar_apply:
            ...
            image_h, image_w = 221, 221 # Default fallback
        ...
        current_size = (221,221) # tiene que ser el size del output del stem
        #current_size = (56,56) # tiene que ser el size del output del stem
    ...
    def get_layer_wise_parameters(self, layer_decay=0, base_lr=None):
        """
