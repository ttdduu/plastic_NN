...
class CustomConvNeXtAttoLC_bias(BaseModel):
    def __init__(self, config, start_from_pretrained_non_lc_weights: bool = True, weights_path=None):
    ...
    def _build_stages(self):
        ...
        self.downsample_layers.append(nn.Sequential(
            LayerNorm(3, eps=1e-6),
            nn.Conv2d(3, self.stem_channels, kernel_size=4, stride=1,bias=True), # User confirmed stride is 1
        ))
        ...
        stem_conv_layer = self.downsample_layers[0][1]
        ...
        for i in range(4):
            ...
            if i > 0:
                ...
                downsample = nn.Sequential(
                    nn.Conv2d(self.dims[i-1], dim, kernel_size=2, stride=2,bias=True),
                    LayerNorm(3, eps=1e-6),
                )
